  ...
  async function exportItem(id, req, res) {
    ...
    try {
      ...
      try {
        ...
        ctx.logger.warn(`dsh-model-viewer: export ${spec.id} failed: ${detail}`);
      ...
      ctx.logger.info(
        `dsh-model-viewer: exported ${id} as ${spec.id} ` +
          `(${bytes.length} bytes, ${reported.mode || "?"}${reported.note ? `, ${reported.note}` : ""})`,
      );
    } catch (err) {
      ctx.logger.warn(`dsh-model-viewer: export error: ${err?.message ?? err}`);
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        try {
          ...
          ctx.logger.warn(`dsh-model-viewer: cadquery_env failed: ${full.error}`);
          return full;
        }
